...
This section walks you through setting up Nova for local development. By the end you will have the API server and web frontend running on your machine.
...
| [Installation](./installation.md) | Clone the repository, install dependencies, and run Nova for the first time |
...
The dev script builds shared packages, starts the Fastify server on `http://127.0.0.1:4010`, waits for a healthy response from `/api/health`, then starts the Next.js frontend on `http://127.0.0.1:3000`.
